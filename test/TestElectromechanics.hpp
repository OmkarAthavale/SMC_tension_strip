...
public:
    ...
    {        
        ...
        std::set_union(fixed_nodes_LHS.begin(), fixed_nodes_LHS.end(),fixed_nodes_RHS.begin(), fixed_nodes_RHS.end(), std::back_inserter(fixed_nodes));
        
        for (st = fixed_nodes.begin(); st != fixed_nodes.end(); ++st) std::cout << ' ' << *st;
        std::cout << '\n';       
