...
#include "MathsCustomFunctions.hpp"
                ...
                pIntracellularStimulus)
    {
        ...
        this->mParameters[0] = 26; // (var_Membrane__C_m) [capacitance_units]
        this->mParameters[1] = 1; // (var_Membrane__Cor) [dimensionless]
        this->mParameters[2] = 0.038899999999999997; // (var_intracellular_Ca__eta) [per_time_units]
        this->mParameters[3] = 0; // (var_ICC_neural__stim_cholinergic) [dimensionless]
        this->mParameters[4] = 0; // (var_ICC_neural__stim_nitrergic) [dimensionless]
    }
    ...
    double CellICCSMCFromCellML::GetIIonic(const std::vector<double>* pStateVariables)
    {
        ...
        const double var_ICC_neural__gbk_neur = var_ICC_neural__E * tanh(var_ICC_neural__F * mParameters[4]) + var_ICC_neural__D; // conductance_units
        const double var_Membrane__C_m_converted = 9.9999999999999995e-7 * mParameters[0]; // uF
        const double var_i_BK__E_K = -72; // voltage_units
        ...
        const double var_i_BK__I_BK = (-var_i_BK__E_K + var_chaste_interface__Membrane__V_m) * var_ICC_neural__gbk_neur * var_d_BK__d_BK; // current_units
    ...
    void CellICCSMCFromCellML::EvaluateYDerivatives(double var_chaste_interface__Time__time, const std::vector<double>& rY, std::vector<double>& rDY)
    {
        ...
        const double var_ICC_neural__beta_neur = var_ICC_neural__B * tanh(var_ICC_neural__C * mParameters[3]) + var_ICC_neural__A; // millimolar_per_time_units
        const double var_ICC_neural__stim_ach = mParameters[3]; // dimensionless
        ...
        const double d_dt_chaste_interface_var_intracellular_Ca__IP_3 = ((1 - pow(var_chaste_interface__Membrane__V_m, var_intracellular_Ca__r) / (pow(var_chaste_interface__Membrane__V_m, var_intracellular_Ca__r) + pow(var_intracellular_Ca__k_v, var_intracellular_Ca__r))) * var_intracellular_Ca__P_MV - var_chaste_interface__intracellular_Ca__IP_3 * mParameters[2] - pow(var_chaste_interface__intracellular_Ca__IP_3, var_intracellular_Ca__u) * var_intracellular_Ca__V_M4 / (pow(var_chaste_interface__intracellular_Ca__IP_3, var_intracellular_Ca__u) + pow(var_intracellular_Ca__k_4, var_intracellular_Ca__u)) + var_ICC_neural__beta_neur) * mParameters[1]; // millimolar / time_units
        ...
        else
        {
            ...
            const double var_i_Ca__G_Ca = pow(var_chaste_interface__intracellular_Ca__Ca_c, var_i_Ca__q) * var_i_Ca__G_MCa / (pow(var_i_Ca__k_Ca, var_i_Ca__q) + pow(var_chaste_interface__intracellular_Ca__Ca_c, var_i_Ca__q)); // conductance_units
            const double var_i_Ca__I_Ca = (-var_i_Ca__E_Ca + var_chaste_interface__Membrane__V_m) * var_i_Ca__G_Ca; // current_units
            d_dt_chaste_interface_var_Membrane__V_m = -(-var_Membrane__I_neural_ICC + var_i_BK__I_BK + var_i_Ca__I_Ca + var_i_Na__I_Na) * mParameters[1] / mParameters[0]; // voltage_units / time_units
        }
